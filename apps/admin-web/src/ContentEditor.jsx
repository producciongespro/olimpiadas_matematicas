...
import { ArrowDown, ArrowUp, Monitor, Plus, Save, Send, Smartphone, Trash2 } from 'lucide-react'
...
export function ContentEditor({ api, busy, run, setConfirmation }) {
  ...
  const addContact = (regionIndex) => setContent((current) => ({ ...current, regions: current.regions.map((region, index) => index === regionIndex ? { ...region, contacts: [...region.contacts, { name: '', emails: [] }] } : region) }))
  ...
  const save = async (event) => {
    ...
    if (selectedKey === 'regional-coordinations') form.set('regions_json', JSON.stringify(content.regions))
