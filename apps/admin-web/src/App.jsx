...
import { ArrowDown, ArrowUp, ImagePlus, LogIn, Save, Trash2 } from 'lucide-react'
...
export function AdminApp() {
  const [token, setToken] = useState(() => sessionStorage.getItem('olcomep_admin_token') || '')
  ...
  const connect = async (event) => {
    event.preventDefault(); sessionStorage.setItem('olcomep_admin_token', token); await load()
  }
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900">
    ...
    <main className="mx-auto max-w-content px-5 py-8 sm:px-8">
      ...
      <nav className="mt-8 flex gap-2 border-b border-slate-300" aria-label="Módulos de medios"><button aria-current={section === 'carousel' ? 'page' : undefined} className="tab" onClick={() => setSection('carousel')} type="button">Carrusel principal</button><button aria-current={section === 'events' ? 'page' : undefined} className="tab" onClick={() => setSection('events')} type="button">Eventos y fotografías</button></nav>
