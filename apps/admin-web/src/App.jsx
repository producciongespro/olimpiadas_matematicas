...
const ContentEditor = lazy(() => import('./ContentEditor.jsx').then((module) => ({ default: module.ContentEditor })))
const UserManagement = lazy(() => import('./UserManagement.jsx').then((module) => ({ default: module.UserManagement })))
...
export function AdminApp({ account, getAccessToken, onLogout, profile }) {
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900">
    ...
    <main className="mx-auto max-w-content px-5 py-8 sm:px-8">
      ...
      {section === 'content' && <Suspense fallback={<p className="empty my-7" role="status">Cargando editor de contenido…</p>}><ContentEditor api={api} busy={busy} run={run} setConfirmation={setConfirmation}/></Suspense>}
      {section === 'users' && <Suspense fallback={<p className="empty my-7" role="status">Cargando gestión de usuarios…</p>}><UserManagement api={api} busy={busy} profile={profile} run={run}/></Suspense>}
